...
function processMessage($message) {
  ...
  if (isset($message['text'])) {
    ...
    if (strpos($text, "/start") === 0) {
      apiRequestJson("sendMessage", array('chat_id' => $chat_id, "text" => "Let's Play the fucking awesome game.", 'reply_markup' => array(
            "inline_keyboard"=>array(
			    array(array("text"=>"How to play","url"=>"http://m.wikihow.com/Play-Tic-Tac-Toe"),
				array("text"=>"History","url"=>"https://en.m.wikipedia.org/wiki/Tic-tac-toe")),
			    array(array("text"=>"Let's Start","switch_inline_query"=>md5(date("YMDms"))))
			)
		)));
    } 
  }
...
function inlineMessage($inline){
	...
	apiRequest("answerInlineQuery",array("inline_query_id"=>$id,"results"=>array(array("type"=>"article","id"=>$query,"title"=>"Tap here","input_message_content"=>array("message_text"=>"Tap on the button to start game","parse_mode"=>"HTML","disable_web_page_preview"=>false),
	    "reply_markup"=>array(
	        "inline_keyboard"=>array(
			  ...
			  if($Tab[$i][$j]["text"]!=" ") apiRequest("answerCallbackQuery",array('callback_query_id'=>$callback_id,'text'=>"You can't choose.",'show_alert'=>false));
                  ...
                  //				  
				    if(Win($Tab)=="😎"||Win($Tab)=="🤔") {
					    ...
					    apiRequest("editMessageText",array("inline_message_id"=>$message_id,"text"=>"First Player:$P1Name(🤔)\nSecond Player:$P2Name(😎)\n\nHaha! LMAO You both are fucking smart!","reply_markup"=>array(
			                "inline_keyboard"=>$Tab 
			            )));  
					    exit;
				    }
